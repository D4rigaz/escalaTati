...
async function createNoturnoWorkers(cycleStartMonth, cycleStartYear) {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    await setCycleStart(emp.id, cycleStartMonth, cycleStartYear);
    workers.push(emp);
  }
  return workers;
}
...
describe('Fix #146 — Jan/2026 w0 (Jan 4–10, globalWi=0=36h, 4 semanas)', () => {
  it('todos os 4 workers Noturno recebem pelo menos 36h em w0', async () => {
    const workers = await createNoturnoWorkers(1, 2026);
    await generateSchedule({ month: 1, year: 2026 });

    for (const w of workers) {
      const entries = await getEntries(w.id);
