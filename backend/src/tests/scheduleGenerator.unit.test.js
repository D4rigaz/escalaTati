...
describe('enforceDailyCoverage', () => {
  ...
  async function insertEntry({ employee_id, date, is_day_off, shift_type_id = null, is_locked = false, notes = null }) {
    await query(
      'INSERT INTO schedule_entries (employee_id, date, is_day_off, shift_type_id, is_locked, notes) VALUES ($1, $2, $3, $4, $5, $6)',
      [employee_id, date, is_day_off, shift_type_id, is_locked, notes]
    );
  }

  // Helper: retorna entry do DB para (employee_id, date)
  async function getEntry(employee_id, date) {
    return (await query(
  ...
  it('converte 2 folgas em turnos sem warning quando dia não tem motorista (passo 1 ambos)', async () => {
    // 2 funcionários com folgas → ambos atribuídos via passo 1 sem forçar → 0 warnings
    const emp1 = await createEmployee(null, { name: 'Ana', setor: 'Transporte Ambulância' });
    const emp2 = await createEmployee(null, { name: 'Beatriz', setor: 'Transporte Ambulância' });
    await insertEntry({ employee_id: emp1.id, date: '2025-01-05', is_day_off: true, shift_type_id: null });
    await insertEntry({ employee_id: emp2.id, date: '2025-01-05', is_day_off: true, shift_type_id: null });
    ...
    await enforceDailyCoverage(employees, sectorMap, shiftTypes, ['2025-01-05'], warnings);

    expect((await getEntry(emp1.id, '2025-01-05')).is_day_off).toBe(false);
    expect((await getEntry(emp2.id, '2025-01-05')).is_day_off).toBe(false);
    expect(warnings).toHaveLength(0);
  });

  it('emite warning sem_motorista_forcado quando único candidato viola restrições de descanso', async () => {
    ...
    const noturno = (await query("SELECT * FROM shift_types WHERE name = 'Noturno'")).rows[0];
    const emp = await createEmployee(null, { name: 'Bruno', setor: 'Transporte Ambulância' });

    await insertEntry({ employee_id: emp.id, date: '2025-01-04', is_day_off: false, shift_type_id: noturno.id });
    await insertEntry({ employee_id: emp.id, date: '2025-01-05', is_day_off: true, shift_type_id: null });
    ...
    const entry = await getEntry(emp.id, '2025-01-05');
    expect(entry.is_day_off).toBe(false);
  });

  it('emite warning sem_motorista quando não há nenhuma folga disponível no dia', async () => {
    const emp = await createEmployee(null, { name: 'Carlos', setor: 'Transporte Ambulância' });
    ...
    await enforceDailyCoverage(employees, sectorMap, shiftTypes, ['2025-01-10'], warnings);

    expect(warnings.some(w => w.type === 'sem_motorista' && w.date === '2025-01-10')).toBe(true);
  });

  it('força funcionário seg_sex no Domingo via passo 3 (emergência) quando não há dom_sab disponível', async () => {
    ...
    const { rows: empRows } = await query(
      "INSERT INTO employees (name, cargo, work_schedule) VALUES ('Diana', 'Motorista', 'seg_sex') RETURNING id",
      []
    );
    const empId = empRows[0].id;
    await query('INSERT INTO employee_sectors (employee_id, setor) VALUES ($1, $2)', [empId, 'Transporte Ambulância']);
    await query('INSERT INTO employee_rest_rules (employee_id, min_rest_hours) VALUES ($1, 24)', [empId]);
    await insertEntry({ employee_id: empId, date: '2025-01-05', is_day_off: true, shift_type_id: null });

    const empObj = (await query('SELECT * FROM employees WHERE id = $1', [empId])).rows[0];
    ...
    const entry = await getEntry(empId, '2025-01-05');
    expect(entry.is_day_off).toBe(false);
  ...
  it('passo 3 não é acionado quando há 2 dom_sab disponíveis no Sábado', async () => {
    // Sáb Jan 4, 2025: 1 seg_sex + 2 dom_sab → 2 dom_sab cobrem o dia, seg_sex preservado
    const { rows: r1 } = await query("INSERT INTO employees (name, cargo, work_schedule) VALUES ('Eduardo', 'Motorista', 'seg_sex') RETURNING id");
    const { rows: r2 } = await query("INSERT INTO employees (name, cargo, work_schedule) VALUES ('Fernanda', 'Motorista', 'dom_sab') RETURNING id");
    const { rows: r3 } = await query("INSERT INTO employees (name, cargo, work_schedule) VALUES ('Gabriela', 'Motorista', 'dom_sab') RETURNING id");
    const [empSegSexId, empDomSab1Id, empDomSab2Id] = [r1[0].id, r2[0].id, r3[0].id];

    for (const id of [empSegSexId, empDomSab1Id, empDomSab2Id]) {
      await query('INSERT INTO employee_sectors (employee_id, setor) VALUES ($1, $2)', [id, 'Transporte Ambulância']);
      await query('INSERT INTO employee_rest_rules (employee_id, min_rest_hours) VALUES ($1, 24)', [id]);
      await insertEntry({ employee_id: id, date: '2025-01-04', is_day_off: true, shift_type_id: null });
    }

    const allEmps = (await query('SELECT * FROM employees WHERE active = TRUE')).rows
      .map(e => ({ ...e, setores: ['Transporte Ambulância'] }));
    const sectorMap = Object.fromEntries(allEmps.map(e => [e.id, ['Transporte Ambulância']]));
    const shiftTypes = (await query('SELECT * FROM shift_types')).rows;
    const warnings = [];

    await enforceDailyCoverage(allEmps, sectorMap, shiftTypes, ['2025-01-04'], warnings);

    // Fernanda e Gabriela (dom_sab) trabalham; Eduardo (seg_sex) preservado
    const entrySegSex  = await getEntry(empSegSexId, '2025-01-04');
    const entryDomSab1 = await getEntry(empDomSab1Id, '2025-01-04');
    const entryDomSab2 = await getEntry(empDomSab2Id, '2025-01-04');
    expect(entryDomSab1.is_day_off).toBe(false);
    expect(entryDomSab2.is_day_off).toBe(false);
    expect(entrySegSex.is_day_off).toBe(true); // Eduardo preservado
    expect(warnings.some(w => w.type === 'sem_motorista_forcado_seg_sex')).toBe(false);
    expect(warnings).toHaveLength(0);
  });

  it('não toca dias que já têm MIN_DAILY_COVERAGE (2) motoristas escalados', async () => {
    const noturno = (await query("SELECT * FROM shift_types WHERE name = 'Noturno'")).rows[0];
    const emp1 = await createEmployee(null, { name: 'Eva', setor: 'Transporte Ambulância' });
    const emp2 = await createEmployee(null, { name: 'Felipe', setor: 'Transporte Ambulância' });
    await insertEntry({ employee_id: emp1.id, date: '2025-01-15', is_day_off: false, shift_type_id: noturno.id });
    await insertEntry({ employee_id: emp2.id, date: '2025-01-15', is_day_off: false, shift_type_id: noturno.id });
    ...
    await enforceDailyCoverage(employees, sectorMap, shiftTypes, ['2025-01-15'], warnings);

    expect(warnings).toHaveLength(0);
    expect((await getEntry(emp1.id, '2025-01-15')).is_day_off).toBe(false);
    expect((await getEntry(emp2.id, '2025-01-15')).is_day_off).toBe(false);
  });

  it('dia com 1 motorista já escalado e 1 folga disponível → segundo atribuído sem warning', async () => {
    // initialCount=1 < MIN_DAILY_COVERAGE=2 → entra no loop e atribui o 2º sem forçar
    const noturno = (await query("SELECT * FROM shift_types WHERE name = 'Noturno'")).rows[0];
    const emp1 = await createEmployee(null, { name: 'Gabi', setor: 'Transporte Ambulância' });
    const emp2 = await createEmployee(null, { name: 'Hugo', setor: 'Transporte Ambulância' });
    await insertEntry({ employee_id: emp1.id, date: '2025-01-06', is_day_off: false, shift_type_id: noturno.id });
    await insertEntry({ employee_id: emp2.id, date: '2025-01-06', is_day_off: true, shift_type_id: null });
    ...
    await enforceDailyCoverage(employees, sectorMap, shiftTypes, ['2025-01-06'], warnings);

    expect((await getEntry(emp2.id, '2025-01-06')).is_day_off).toBe(false); // Hugo atribuído
    expect(warnings).toHaveLength(0); // sem forçar → sem warning
  });

  it('dia com 1 motorista escalado, único candidato viola descanso → segundo_motorista_forcado', async () => {
    // emp1 já trabalhando; emp2 tem folga mas violaria descanso → forçado via passo 2
    const noturno = (await query("SELECT * FROM shift_types WHERE name = 'Noturno'")).rows[0];
    const emp1 = await createEmployee(null, { name: 'Iara', setor: 'Transporte Ambulância' });
    const emp2 = await createEmployee(null, { name: 'Jonas', setor: 'Transporte Ambulância' });
    await insertEntry({ employee_id: emp1.id, date: '2025-01-06', is_day_off: false, shift_type_id: noturno.id });
    // Jonas trabalhou no noturno anterior → canAssignShift rejeita no passo 1
    await insertEntry({ employee_id: emp2.id, date: '2025-01-05', is_day_off: false, shift_type_id: noturno.id });
    await insertEntry({ employee_id: emp2.id, date: '2025-01-06', is_day_off: true, shift_type_id: null });
    ...
    expect((await getEntry(emp2.id, '2025-01-06')).is_day_off).toBe(false); // Jonas forçado
  ...
  it('dia com 0 motoristas e apenas 1 folga → 1 atribuído + cobertura_minima_insuficiente', async () => {
    // Só 1 funcionário disponível como folga → preenche 1, não consegue 2
    const emp = await createEmployee(null, { name: 'Karina', setor: 'Transporte Ambulância' });
    await insertEntry({ employee_id: emp.id, date: '2025-01-07', is_day_off: true, shift_type_id: null });
    ...
    await enforceDailyCoverage(employees, sectorMap, shiftTypes, ['2025-01-07'], warnings);

    expect((await getEntry(emp.id, '2025-01-07')).is_day_off).toBe(false); // Karina atribuída
  ...
  it('dia com 1 motorista escalado e sem folgas disponíveis → cobertura_minima_insuficiente', async () => {
    // emp1 trabalha; emp2 está de férias (locked notes=Férias) → sem candidatos para 2º
    const noturno = (await query("SELECT * FROM shift_types WHERE name = 'Noturno'")).rows[0];
    const emp1 = await createEmployee(null, { name: 'Leo', setor: 'Transporte Ambulância' });
    const emp2 = await createEmployee(null, { name: 'Marta', setor: 'Transporte Ambulância' });
    await insertEntry({ employee_id: emp1.id, date: '2025-01-08', is_day_off: false, shift_type_id: noturno.id });
    await insertEntry({ employee_id: emp2.id, date: '2025-01-08', is_day_off: true, shift_type_id: null, notes: 'Férias' });
    ...
    await enforceDailyCoverage(employees, sectorMap, shiftTypes, ['2025-01-08'], warnings);

    expect((await getEntry(emp1.id, '2025-01-08')).is_day_off).toBe(false); // Leo permanece
    expect((await getEntry(emp2.id, '2025-01-08')).is_day_off).toBe(true); // Marta em férias intocada
